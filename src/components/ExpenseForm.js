...
onAmountChange= (e) => {
    ...
    //belwo matches empty string and regex with it
    if(!amount || amount.match(/^\d{1,}(\.\d{0,2})?$/)){
         this.setState(()=>({amount}))
       }
   
}
onDateChange = (date) => { //, it activates a calendar again, it takes an argument(can be Date or Now and any name) in the form of moment and gives the back the date to reset the date
    //not letting user to clear date field, now setting onChange only when there's date already present there, thus forcing date to be present all the time and n opt letting user to chnage it at all, and change only if there's date
    if(date){
...
onFocusChange =  ({focused}) => { //as given in official docs, destructuring the focused property
    ...
    render(){
        return(
          ...
          <form onSubmit={this.onSubmit}>
            <input type="text" name="description" placeholder="Description" value={this.state.description} onChange={this.onDescriptionChange} autoFocus/>
...
<textarea name="note" placeholder="Add a note for your expense(optional)" value={this.state.note} onChange={this.onNoteChange}></textarea>
